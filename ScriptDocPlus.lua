...
function ScriptDocPlus:ClampLineNumber(lineNum:int)
	return math.clamp(lineNum, 1, self:GetLineCount())
end

function ScriptDocPlus:Paste(text, lineNum, fromCurrent)
...
function ScriptDocPlus:GetLine(lineNum:int, fromCurrent:boolean)
...
function ScriptDocPlus:WasReturnPressed()
	local changes = self.changes
	if not changes then warn "please pass in the \"changes\" parameter from the \"TextDocumentDidChange\" function."; return false end
	if #changes == 0 then return false end
	if changes[1].text == "\n" then return true end
	return false
end

function ScriptDocPlus:GetChangedText()
	local changes = self.changes
	if not changes then warn "please pass in the \"changes\" parameter from the \"TextDocumentDidChange\" function."; return "" end
...
function ScriptDocPlus:GetText(startLine:int, startCharacter:int, endLine:int, endCharacter:int)
...
function ScriptDocPlus:ForceSetSelectionAsync(cursorLine:number, cursorCharacter:number, anchorLine:int, anchorCharacter:int)
	return self.document:ForceSetSelectionAsync(cursorLine, cursorCharacter, anchorLine, anchorCharacter)
end
function ScriptDocPlus:RequestSetSelectionAsync(cursorLine:number, cursorCharacter:number, anchorLine:int, anchorCharacter:int)
